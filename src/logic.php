...
class Logic implements MessageComponentInterface {
    ...
    private bool $roundEnding    = false; // FIX#3: guard against double-call
        ...
        if (count($searching) >= self::MAX_PLAYERS) {
            ...
            foreach ($matched as $mp) {
                ...
                $mp['conn']->send($this->encode([
                    'type'=>'MATCH_FOUND','opponent'=>$opp['username']??'?','opponentIcon'=>$opp['icon']??'fa-robot'  // FIX#2: array not object
                ]));
            }
    ...
    private function startGame(): void {
        ...
        $this->roundEnding  = false; // FIX#3

        foreach ($this->players as &$p) {
            $p['score']=0;  // FIX#1: was string '0' $p['reactionLog']=[]; $p['combo']=0;
    ...
    private function startRound(): void {
        ...
        $this->activeItems = [];
        $this->roundEnding = false; // FIX#3: reset guard
    ...
    private function checkRoundEnd(): void {
        ...
        if ($this->roundEnding) return; // FIX#3: guard double-call
    ...
    private function saveToDatabase(array $stats): void {
        ...
        try {
            ...
            // Round logs
            // FIX#10: insert one row per player per round with round-specific score
            if (!empty($this->roundLog)) {
                ...
                foreach ($this->roundLog as $r) {
                    foreach ($stats as $p) {
                        // round_score = score at this round (from scores snapshot)
                        // reaction_time = player avg for full game (best available per-game data)
                        $roundScore = $r['scores'][$p['username']] ?? 0;
                        $stmtRound->execute([
                            ':u' => $p['username'],
                            ':t' => $p['avgTime'] ?? 0,
                            ':s' => $roundScore,
    ...
    private function broadcastPlayerList(): void {
        $list = array_map(fn($p) => [
            ...
            'isGuest'=>$p['isGuest'],'ready'=>$p['isReady'],'level'=>1,
        ], $this->players);
